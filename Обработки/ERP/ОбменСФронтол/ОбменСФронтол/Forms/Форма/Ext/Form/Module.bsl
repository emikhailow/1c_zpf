		...
		Если КассаСоответствуетСкладу(СтрокаКасса.ПодключаемоеОборудование, Склад) Тогда
			...
			Если ЗначениеЗаполнено(ДанныеУстройства.ФайлОтчета)Тогда
				...
				Иначе
					...
					Если СтруктураДанные.Свойство("ДатаНачала") Тогда
						МассивСвойств.Добавить("ДатаНачала");
					КонецЕсли;
					Если СтруктураДанные.Свойство("ДатаКонца") Тогда
						МассивСвойств.Добавить("ДатаКонца");
					КонецЕсли;
					Если СтруктураДанные.Свойство("Время") Тогда
						МассивСвойств.Добавить("Время");
					КонецЕсли;
					
					Если МассивСвойств.Количество() Тогда
						ЗаполнитьЗначенияСвойств(СтруктураПараметры, СтруктураДанные, СтрСоединить(МассивСвойств, ","));
					КонецЕсли;
...
Процедура ПеренестиТоварыИОплатыИзОРПВОРП(ОРПСобственнаяПродукцияОбъект, ОРППокупнаяПродукцияОбъект)
	...
	Если Не РезультатНоменклатураПокупная.Пустой() Тогда
		...
		ОплатаПлатежнымиКартами	= ОРПСобственнаяПродукцияОбъект.ОплатаПлатежнымиКартами.Итог("Сумма");
		ПрочаяОплата			= ОРПСобственнаяПродукцияОбъект.битзпф_ОплатаПрочимиТипами.Итог("Сумма");
		ОплатаВсего				= ОплатаПлатежнымиКартами + ПрочаяОплата;
				
		НеобходимоПеренести = Макс(ОплатаВсего - СобственнаяПродукцияСумма, 0);
		
		Если НеобходимоПеренести Тогда
			
			МассивУдаляемыхСтрок = Новый Массив;
			
			Для Каждого Строка Из ОРПСобственнаяПродукцияОбъект.ОплатаПлатежнымиКартами Цикл
				
				КПереносу = Мин(НеобходимоПеренести, Строка.Сумма);
				
				НоваяСтрока = ОРППокупнаяПродукцияОбъект.ОплатаПлатежнымиКартами.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка, , "Сумма");
				НоваяСтрока.Сумма 	= КПереносу;
				Строка.Сумма 		= Строка.Сумма - КПереносу;

				НеобходимоПеренести = НеобходимоПеренести - КПереносу;
				
				Если Не НеобходимоПеренести Тогда
					Прервать;	
				КонецЕсли;
			...
			Для Каждого УдаляемаяСтрока Из МассивУдаляемыхСтрок Цикл
				ОРПСобственнаяПродукцияОбъект.ОплатаПлатежнымиКартами.Удалить(УдаляемаяСтрока);				
			КонецЦикла;
	...
	Для Каждого ДокументКПроведению Из МассивДокументовКПроведению Цикл
		...
		Попытка
			...
			Иначе	
				...
				РежимПроведения = ?(НачалоДня(ДокументКПроведению.Дата) = НачалоДня(ТекущаяДата()), 
				РежимПроведенияДокумента.Оперативный,
				РежимПроведенияДокумента.Неоперативный);
